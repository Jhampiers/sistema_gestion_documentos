...
class ArchivosModel extends Query{
    ...
    {
        $sql = "SELECT * FROM carpetas WHERE id_usuario = $id_usuario AND estado = 1 ORDER BY id DESC LIMIT $desde, $porPagnina";
        return $this->selectAll($sql);
    }

    public function getTotalCarpetas($id_usuario)
    {
        $sql = "SELECT COUNT(id) AS total FROM carpetas WHERE id_usuario = $id_usuario AND estado = 1";
